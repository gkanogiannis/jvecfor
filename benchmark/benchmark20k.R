...
#  Uses TENxBrainData20k (20 k-cell subset) for fast local development.
#
...
#
#  Method  :
#    1. Download & log-normalise the 20 k-cell dataset.
...
HNSW_M           <- 32L               # max connections per node (default 16; 32 improves recall)
HNSW_EF_SEARCH   <- 0L                # search beam width (0 = auto: max(k+1, 3*k) = 45 for k=15)
HNSW_OVERSAMPLE  <- 2.0               # oversampling factor: fetches ceil(ef*factor) candidates
...
N_VALUES  <- as.integer(c(1e3, 5e3, 1e4, 2e4))

N_REPS    <- 3L                       # timing repetitions per (n, method, phase)
PLOT_MAX_N   <- 20000L                # skip igraph plot for n above this threshold
...
if (file.exists(PCA_CACHE)) {
    ...
    message("Downloading TENxBrainData20k ...")
    ...
    message("  RandomParam: ~2 HDF5 passes. No in-memory materialisation.")
    set.seed(42L)
...
for (n in N_VALUES) {
    ...
    message(sprintf("  Saved: graphs20k/ outputs for n=%s", format(n, big.mark = ",")))
...
params_subtitle <- sprintf(
    "Dataset: TENxBrainData20k  |  k=%d  %d PCs  %d threads  M=%d ef=%d oversample=%.2f  pq=%d",
...
ggsave("benchmark_results_20k.pdf", p, width = 10, height = 5.5)
message("Saved: benchmark_results_20k.pdf")
...
ggsave("benchmark_memory_20k.pdf", p_mem, width = 10, height = 5.5)
message("Saved: benchmark_memory_20k.pdf")
