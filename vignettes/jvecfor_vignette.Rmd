...
requires no separate installation step beyond having Java 21+ on your `PATH`.
...
jvecfor requires **Java ≥ 21** on `PATH`. The JAR uses the
`jdk.incubator.vector` module for SIMD acceleration. Check your installation:

```{r check-java, eval=FALSE}
system2("java", "--version")  # should print openjdk 21 or higher
```

If `java` is not found, install [Eclipse Temurin](https://adoptium.net) or
any OpenJDK 21+ distribution and ensure the `java` binary is on your `PATH`.
...
err_tbl <- data.frame(
    "Error message" = c(
        ...
        "`Java >= 21 is required (found Java X)`",
    ...
    "Resolution" = c(
        paste("Install OpenJDK 21+ (adoptium.net);",
                "re-launch R after updating PATH."),
        paste("Upgrade to OpenJDK 21+;",
